...
<template>
  <interact
    ...
    :style="style"
    @dragmove="dragmove"
    :class="{ fullscreen: $store.getters.isFullscreenStickies }"
  >
    <div
      id="container"
      :class="{
        fullscreen: $store.getters.isFullscreenStickies,
        close: !$store.getters.isShownStickies,
      }"
    >
      <div
        id="mydivheader"
        class="top-bar"
        v-on:dblclick="$store.commit('toggleFullscreenStickies')"
      >
        <div class="close-button" v-on:click="closeStickies"></div>
      </div>
      <textarea
        v-model="notepadDataLocally"
